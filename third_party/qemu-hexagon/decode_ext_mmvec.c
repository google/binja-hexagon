...
static void check_dv_instruction(HVXResource *resources, int *ilist,
                                 int num_insn, Packet *packet,
                                 unsigned int attribute, HVXResource resource0,
                                 HVXResource resource1) {
  int current_insn = 0;
...
static void check_dv_instruction2(HVXResource *resources, int *ilist,
                                  int num_insn, Packet *packet,
                                  unsigned int attribute, HVXResource resource0,
                                  HVXResource resource1, HVXResource resource2,
                                  HVXResource resource3) {
  int current_insn = 0;
  /* Loop on vector instruction count */
  for (current_insn = 0; current_insn < num_insn; current_insn++) {
    /* valid instruction */
...
static void check_umem_instruction(HVXResource *resources, int *ilist,
                                   int num_insn, Packet *packet) {
  int current_insn = 0;
  /* Loop on vector instruction count */
  for (current_insn = 0; current_insn < num_insn; current_insn++) {
    ...
    if (ilist[current_insn] > -1) {
      ...
      if (GET_ATTRIB(opcode, A_CVI_VP) && GET_ATTRIB(opcode, A_CVI_VM)) {
        ...
        if ((resources[HVX_RESOURCE_LOAD] + resources[HVX_RESOURCE_STORE] +
...
/* Memory instructions */
static void check_mem_instruction(HVXResource *resources, int *ilist,
                                  int num_insn, Packet *packet) {
  int current_insn = 0;
  /* Loop on vector instruction count */
  for (current_insn = 0; current_insn < num_insn; current_insn++) {
    ...
    if (ilist[current_insn] > -1) {
      ...
      if (GET_ATTRIB(opcode, A_CVI_VM)) {
        ...
        if (!(GET_ATTRIB(opcode, A_CVI_TMP) && GET_ATTRIB(opcode, A_LOAD)) &&
            !(GET_ATTRIB(opcode, A_CVI_NEW) && GET_ATTRIB(opcode, A_STORE))) {
 ...
 */
static void check_instruction1(HVXResource *resources, int *ilist, int num_insn,
                               Packet *packet, unsigned int attribute,
...
static void check_instruction2(HVXResource *resources, int *ilist, int num_insn,
                               Packet *packet, unsigned int attribute,
...
static void check_instruction4(HVXResource *resources, int *ilist, int num_insn,
                               Packet *packet, unsigned int attribute,
                               HVXResource resource0, HVXResource resource1,
...
static void check_4res_instruction(HVXResource *resources, int *ilist,
                                   int num_insn, Packet *packet) {
  int current_insn = 0;
  for (current_insn = 0; current_insn < num_insn; current_insn++) {
    if (ilist[current_insn] > -1) {
      ...
      if (GET_ATTRIB(opcode, A_CVI_4SLOT)) {
        int available_resource =
            resources[HVX_RESOURCE_PERM] + resources[HVX_RESOURCE_SHIFT] +
            resources[HVX_RESOURCE_MPY0] + resources[HVX_RESOURCE_MPY1];
...
static void decode_populate_cvi_resources(Packet *packet) {
  int i, num_insn = 0;
  int vlist[4] = {-1, -1, -1, -1};
  HVXResource hvx_resources[8] = {FREE, FREE, FREE, FREE,
                                  FREE, FREE, FREE, FREE}; /* All Available */
  ...
  check_dv_instruction(hvx_resources, vlist, num_insn, packet, A_CVI_VP_VS,
                       HVX_RESOURCE_SHIFT, HVX_RESOURCE_PERM);
  /* Single vector permute can only go to permute resource */
  check_instruction1(hvx_resources, vlist, num_insn, packet, A_CVI_VP,
                     HVX_RESOURCE_PERM);
  /* Single vector permute can only go to permute resource */
  check_instruction1(hvx_resources, vlist, num_insn, packet, A_CVI_VS,
                     HVX_RESOURCE_SHIFT);

  /* Try to insert double vector multiply */
  check_dv_instruction(hvx_resources, vlist, num_insn, packet, A_CVI_VX_DV,
                       HVX_RESOURCE_MPY0, HVX_RESOURCE_MPY1);
  /* Try to insert double capacity mult */
  check_dv_instruction2(hvx_resources, vlist, num_insn, packet, A_CVI_VS_VX,
                        HVX_RESOURCE_SHIFT, HVX_RESOURCE_MPY0,
                        HVX_RESOURCE_PERM, HVX_RESOURCE_MPY1);
  /* Single vector permute can only go to permute resource */
  check_instruction2(hvx_resources, vlist, num_insn, packet, A_CVI_VX,
                     HVX_RESOURCE_MPY0, HVX_RESOURCE_MPY1);
  /* Try to insert double vector alu */
  check_dv_instruction2(hvx_resources, vlist, num_insn, packet, A_CVI_VA_DV,
                        HVX_RESOURCE_SHIFT, HVX_RESOURCE_PERM,
                        HVX_RESOURCE_MPY0, HVX_RESOURCE_MPY1);

  check_mem_instruction(hvx_resources, vlist, num_insn, packet);
  /* single vector alu can go on any of the 4 pipes */
  check_instruction4(hvx_resources, vlist, num_insn, packet, A_CVI_VA,
                     HVX_RESOURCE_SHIFT, HVX_RESOURCE_PERM, HVX_RESOURCE_MPY0,
                     HVX_RESOURCE_MPY1);
}

static void check_new_value(Packet *packet) {
  ...
  for (i = 1; i < packet->num_insns; i++) {
    ...
    if (GET_ATTRIB(use_opcode, A_DOTNEWVALUE) &&
      ...
      int use_regidx =
          strchr(opcode_reginfo[use_opcode], 's') - opcode_reginfo[use_opcode];
      /*
...
static void decode_mmvec_move_cvi_to_end(Packet *packet, int max) {
  int i;
  for (i = 0; i < max; i++) {
    if (GET_ATTRIB(packet->insn[i].opcode, A_CVI)) {
...
static void decode_shuffle_for_execution_vops(Packet *packet) {
  /*
   * Sort for V.new = VMEM()
...
static void decode_hvx_packet_contents(Packet *pkt) {
